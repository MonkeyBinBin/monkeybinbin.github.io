      ...
      <nav v-if="totalPages > 1" class="pagination-nav" aria-label="文章分頁導覽">
        <div class="pagination-wrapper">
          ...
          <div class="pagination-numbers">
            <!-- 第一頁 -->
            <NuxtLink v-if="showFirstPage" to="/" class="pagination-number active"> 1 </NuxtLink>
            ...
            <!-- 最後一頁 -->
            <NuxtLink v-if="showLastPage" :to="`/page/${totalPages}`" class="pagination-number">
              {{ totalPages }}
            </NuxtLink>
          </div>
...
const { data: postsData, error } = await useAsyncData('posts-page-1', () =>
  api().getArticles(POSTS_PER_PAGE, 0)
);
...
const showLastPage = computed(
  () => currentPage < totalPages.value - visibleRange || totalPages.value <= 5
);
