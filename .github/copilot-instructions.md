...
- 單一 PR 應只處理**一個邏輯變更**，不要把 bug fix、refactor、流程調整綁在一起
- 若文件、測試或 template 是為了支援同一主題的主變更，且無法獨立成立為另一個邏輯變更，可與主變更一併提交
- 若變更同時涵蓋多個獨立面向，即使都與同一個 issue 有關，也應拆成多個 PR，而不是只在 PR description 拆解階段
- 檔案數量不是唯一判準；但只要變更已明顯提高 review 或 rollback 成本，就應優先拆分
